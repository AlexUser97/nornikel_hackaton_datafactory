...
def result_to_csv(result: dict) -> str:
    ...
    if result.get("astm_applicable", True):
        w.writerow(["Метрика", "Балл зерна ASTM E112", result.get("astm_number", ""), "балл"])
    ...
    if pr:
        ...
        w.writerow(["Доля площади", "Тальк (зона оталькования)", _p("тальк (зона оталькования)"), "%"])
        w.writerow(["Доля площади", "Оксиды (магнетит)", _p("оксиды (магнетит)"), "%"])
        w.writerow(["Доля площади", "Срастания (сульфиды)", _p("срастания (рудные вкрапленники)"), "%"])
        if pr.get("срастания (рудные вкрапленники)") is not None:
            ...
            w.writerow(["Доля площади", "Срастания внутри талька", _p("срастания внутри талька"), "%"])
