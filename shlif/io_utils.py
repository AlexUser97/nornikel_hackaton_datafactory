...
def result_to_csv(result: dict) -> str:
    ...
    if pr:
        ...
        w.writerow(["Доля площади", "Тальк в нерудной части (класс >10%)", _p("тальк в нерудной части"), "%"])
        w.writerow(["Доля площади", "Тальк (зона оталькования, % кадра)", _p("тальк (зона оталькования)"), "%"])
        w.writerow(["Доля площади", "Оксиды (магнетит)", _p("оксиды (магнетит)"), "%"])
        w.writerow(["Доля площади", "Срастания (обычные + тонкие)", _p("срастания (рудные вкрапленники)"), "%"])
